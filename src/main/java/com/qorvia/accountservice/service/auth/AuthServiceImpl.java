...
import com.qorvia.accountservice.client.NotificationRabbitMqClient;
...
public class AuthServiceImpl implements AuthService {
    ...
    private final NotificationRabbitMqClient notificationRabbitMqClient;
    ...
    public UserDTO createAccount(RegisterRequest request) {
        ...
        notificationRabbitMqClient.sendOtp(request.getEmail());
    ...
    public ResponseEntity<ApiResponse<OtpResponse>> verifyEmail(OtpRequest otpRequest, HttpServletResponse servletResponse) {
        if (!userRepository.existsByEmail(otpRequest.getEmail())) {
            log.error("Email does not exist: {}", otpRequest.getEmail());
            return ResponseUtil.buildResponse(HttpStatus.BAD_REQUEST, "Email does not exist", null);
        }

        try {

            VerifyOtpMessageResponse response = notificationRabbitMqClient.verifyOtp(otpRequest.getEmail(), otpRequest.getOtp());

            if (response != null && response.getIsVerified()) {
                ...
                log.info("Email successfully verified: {}", otpRequest.getEmail());
                ...
                log.warn("Email verification failed for email: {}", otpRequest.getEmail());
            ...
            log.error("Exception occurred during email verification for email: {}", otpRequest.getEmail(), e);
    ...
    public ResponseEntity<String> organizerEmailVerificationSendRequest(String email) {
        try {
            ...
            notificationRabbitMqClient.organizerEmailVerificationRequest(email);
    ...
    public ResponseEntity<ApiResponse<OrganizerVerificationResponse>> organizerVerificationTokenVerify(
        ...
        try {
            ...
            OrganizerTokenVerifyMessageResponse verificationResponse = notificationRabbitMqClient.organizerTokenVerify(request.getToken());

            if (verificationResponse.getEmail() != null) {

                log.info("here I am getting the organizer verification token and it's response mail is : {}", verificationResponse.getEmail());

                Organizer organizer = organizerRepository.findByEmail(verificationResponse.getEmail()).get();
                ...
                OrganizerVerificationResponse verificationData = new OrganizerVerificationResponse();
                verificationData.setEmail(verificationResponse.getEmail());
    ...
    public ResponseEntity<?> forgotPassword(ForgotPasswordRequest forgotPasswordRequest) {
        ...
        if (userInfo.isPresent()) {
            if (!userInfo.get().getIsGoogleAuth()) {
                notificationRabbitMqClient.sendOtp(email);
    ...
    @Override
    public ResponseEntity<?> forgotPasswordReset(ForgotPasswordResetRequest passwordResetRequest) throws TimeoutException, IOException {
        String email = passwordResetRequest.getEmail();
        String otp = passwordResetRequest.getOtp();
        String newPassword = passwordResetRequest.getPassword();

//        try {
            VerifyOtpMessageResponse verifyOtpResponse = notificationRabbitMqClient.verifyOtp(email, otp);
...
//        }
        ...
        if (verifyOtpResponse != null && verifyOtpResponse.getIsVerified()) {
            Optional<UserInfo> userInfo = userRepository.findByEmail(email);
            if (userInfo.isPresent()) {
